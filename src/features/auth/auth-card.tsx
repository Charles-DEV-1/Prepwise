...
import { phoneAuthSchema, otpSchema, type PhoneAuthValues, type OtpValues } from "@/lib/validations";
import { signInWithGoogle, signInWithPhone, verifyPhoneOtp } from "@/services/auth";
...
export function AuthCard({ mode }: { mode: "login" | "signup" }) {
  ...
  const phoneForm = useForm<PhoneAuthValues>({ resolver: zodResolver(phoneAuthSchema), defaultValues: { phone: "" } });
  const otpForm = useForm<OtpValues>({ resolver: zodResolver(otpSchema), defaultValues: { phone: "", token: "" } });
  ...
  return (
    <Card className="w-full max-w-md shadow-soft">
      <CardHeader>
        ...
        <CardDescription>Use phone OTP or Google to continue to Prepwise.</CardDescription>
      </CardHeader>
      <CardContent>
        {!otpSent ? (
          <form className="space-y-4" onSubmit={phoneForm.handleSubmit(requestOtp)}>
            <div className="space-y-2">
              <Label htmlFor="phone">Phone number</Label>
              <Input id="phone" placeholder="+2348012345678" {...phoneForm.register("phone")} />
          ...
          <form className="space-y-4" onSubmit={otpForm.handleSubmit(submitOtp)}>
            <div className="space-y-2">
              ...
              <Input id="token" inputMode="numeric" placeholder="123456" {...otpForm.register("token")} />
              {otpForm.formState.errors.token && <p className="text-xs text-destructive">{otpForm.formState.errors.token.message}</p>}
            </div>
            <Button className="w-full" disabled={isLoading}>
